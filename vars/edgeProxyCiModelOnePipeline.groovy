        ...
        entityDeploymentInfos.each {
                        ...
                        stage('deploy-proxy') {
                            ...
                            withCredentials([file(credentialsId: it.org, variable: 'serviceAccount')]) {
				               ...
				               // echo "${bearer}"
                                sh '''
				mvn -X package apigee-enterprise:deploy -Phybrid-apiproxy -Dorg=sidgs-hybrid -Denv=dev-4 -Dfile=$serviceAccount -s /usr/share/maven/conf/settings.xml
    '''
                            }
...
//                    }
            ...
            if (DefaultConfigService.instance.steps.release) {
                        stage('upload-artifact') {
                            ...
                            sh 'mvn -X deploy -s /usr/share/maven/conf/settings.xml'
                        }
                        }
